...
if __name__ == '__main__':
    ...
    max_video, max_video_ratio, max_video_size = max(zip(videos, video_ratios, video_sizes), key=lambda v: v[0])
    min_video, min_video_ratio, min_video_size = min(zip(videos, video_ratios, video_sizes), key=lambda v: v[0])

    max_image, max_image_ratio, max_image_size = max(zip(images, image_ratios, image_sizes), key=lambda v: v[0])
    min_image, min_image_ratio, min_image_size = min(zip(images, image_ratios, image_sizes), key=lambda v: v[0])
    ...
    total_file_out = sum((out_size for _, out_size in image_sizes))
